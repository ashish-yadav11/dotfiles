...
usbmount() {
    echo "$drives_to_mount" | $dmenu_command -p "Which drive(s) to mount?" | cut -d" " -f1 |
        while read -r chosen ; do
            notify-send -t 2000 " USB mounter" "$(udisksctl mount -b "$chosen")"
        done
}

usbunmount() {
    echo "$drives_to_unmount" | $dmenu_command -p "Which drive(s) to unmount?" | cut -d" " -f1 |
        while read -r chosen ; do
            notify-send -t 2000 " USB mounter" "$(udisksctl unmount -b "$chosen")"
        done
}

asktype() {
    M=$(echo "$drives_to_mount" | awk 'BEGIN {ORS=""} {if (!x) {x=1; print $0} else {print ", "$0}}')
    U=$(echo "$drives_to_unmount" | awk 'BEGIN {ORS=""} {if (!x) {x=1; print $0} else {print ", "$0}}')
    case "$(echo "Mount: $M\nUnmount: $U" | dmenu -i -matching fuzzy -no-custom -p "What do you want me to do?")" in
        Mount*) 
            if [ "$(echo "$drives_to_mount" | wc -l)" = 1 ] ; then
                notify-send -t 2000 " USB mounter" "$(udisksctl mount -b "${drives_to_mount%% *}")"
            else
                usbmount
            fi
        ;;
	Unmount*)
            if [ "$(echo "$drives_to_unmount" | wc -l)" = 1 ] ; then
                notify-send -t 2000 " USB mounter" "$(udisksctl unmount -b "${drives_to_unmount%% *}")"
            else
                usbunmount
            fi
        ;;
